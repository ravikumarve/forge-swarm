...
class CodeSandbox:
    ...
    def execute(self, code: str) -> Dict[str, Any]:
        ...
        Security layers:
        1. RestrictedPython compile_restricted() — restricts syntax (imports, loops, attrs)
        2. __builtins__ stripped from globals — prevents builtin escape
        3. Signal-based timeout — prevents infinite loops
        4. Whitelist-only _print_ — only output function allowed
        """
        if not self.enabled:
            return {
                "output": "",
        ...
        # Build restricted globals: strip __builtins__ completely
        restricted_globals = {
            "__builtins__": {},  # Explicitly empty — blocks builtin escape
            "_print_": print,
            "_getattr_": safe_globals.get("_getattr_", getattr),
            "_stringify_": safe_globals.get("_stringify_", str),
        }
        ...
        def timeout_handler(_signum, _frame):
            raise SandboxTimeout(f"Execution timed out after {self.timeout}s")
...
class MCPToolManager:
    ...
    def _exec_python(self, code: str) -> str:
        ...
        if self._sandbox is None:
            from forge_swarm_core import CodeSandbox
